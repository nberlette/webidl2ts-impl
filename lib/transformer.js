...
function upsertDefinition(map, rootType) {
  ...
  if (!rootType.partial) {
    ...
    existing.extAttrs = [...existing.extAttrs, ...(rootType.extAttrs ?? [])];
  }
...
function emitIndexSignature(member, generatedNames) {
  ...
  const keyType = argument.idlType.idlType === "unsigned long" ? "number" : "string";
...
function emitDefinitionFile(name, kind, definition, registry, generatedNames, unresolvedNames) {
  ...
  switch (kind) {
    ...
    case "interfaces": {
      const mixins = [...(registry.includes.get(name) ?? [])].sort();
...
export default class Transformer {
  ...
  async generate(outputDir) {
    ...
    for (const { key, name } of registry.order) {
